 ...
 * - Registers OneSignal player_id to the backend when the app opens.
 * - Retries if user wasn't logged in yet.
 * - Call window.sosBridge.retryToken() from your login success handler.
 */
(function () {
  'use strict';
  ...
  function saveToken(playerId) {
    ...
    .then(function (d) {
      if (d.success) {
        ...
        console.warn('[SOS] Token not saved:', d.message);
      }
    })
    .catch(function (e) { console.warn('[SOS] Token error:', e); });
  }
  ...
  window.sosBridge = {
    retryToken: function () {
      if (_pendingToken && !_saved) { _saved = false; saveToken(_pendingToken); }
    }
  };

  document.addEventListener('DOMContentLoaded', function () {
    if (_pendingToken && !_saved) {
      setTimeout(function () { if (!_saved) saveToken(_pendingToken); }, 1000);
    }
  });
  ...
  function showSosBanner(payload) {
    ...
    banner.innerHTML =
      ...
      '<div style="flex:1;min-width:0">' +
        ...
        '<div style="font-size:.82rem;opacity:.92">' + esc(body) + '</div>' +
