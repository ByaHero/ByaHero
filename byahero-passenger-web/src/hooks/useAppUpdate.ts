...
export function useAppUpdate() {
  ...
  useEffect(() => {
    ...
    async function checkVersion() {
      // For web, app updates usually happen on page reload.
      // But we keep the exact logic if we want to prompt them to reload.
      try {
        const baseUrl = await getServerUrl();
        const response = await fetch(`${baseUrl}/api/app-version?app=passenger-web`);
        ...
        if (data.success && data.latest_version) {
          ...
          if (needsUpdate && isMounted) {
            setUpdateInfo({
              ...
              download_url: data.download_url || window.location.origin,
        ...
        console.log('[useAppUpdate Web] Backend check skipped:', err);
      }
    }
