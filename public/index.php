...
<head>
  ...
  <style>
    /* --- BRANDING OVERRIDES --- */
    :root {
      --bs-primary: #1e3a8a; /* Your Brand Blue */
    ...
    .w-40px { width: 40px; }
    .h-40px { height: 40px; }
    .rounded-top-4 { border-top-left-radius: 1.5rem; border-top-right-radius: 1.5rem; }

    /* --- MOBILE COMPONENTS --- */
    .map-overlay { z-index: 1000; }
    .filter-pill { cursor: pointer; font-size: 0.8rem; letter-spacing: 0.5px; }
    ...
    .drag-handle { width: 40px; height: 5px; border-radius: 10px; }

    /* Timeline Graphics */
    .timeline-container { height: 12px; border-radius: 12px; }
    .timeline-progress { transition: width 600ms ease; }
    .timeline-icon { top: -12px; transform: translateX(-50%); padding: 2px; }
    .stop-point { top: -6px; width: 18px; height: 18px; border: 3px solid #e6eefc; z-index: 2; }
...
<body class="bg-light">
  ...
  <div class="d-lg-none d-flex flex-column vh-100 w-100">
    ...
    <div class="bg-primary d-flex align-items-center rounded-bottom-4 px-3 shadow-sm h-60px w-100 position-absolute top-0 start-0 z-3">
    ...
    <div class="flex-grow-1 position-relative" style="min-height: 0;">
      ...
      <div class="position-absolute mt-5 top-0 start-0 end-0 p-3 d-flex justify-content-between align-items-center map-overlay">
        <button class="btn btn-light rounded-circle shadow p-0 h-40px w-40px d-flex align-items-center justify-content-center border-0"
          data-bs-toggle="modal" data-bs-target="#settingsModal">
          <span class="material-symbols-rounded">settings</span>
        </button>

        <div class="bg-white rounded-pill shadow px-4 py-2 d-flex align-items-center gap-2 fw-bold text-dark filter-pill"
      ...
      <button class="position-absolute start-50 translate-middle-x bottom-0 mb-4 btn btn-light rounded-pill shadow px-4 py-2 d-flex align-items-center gap-2 text-primary fw-bold border-0 map-overlay"
        style="z-index: 1000;">
  ...
  <div class="bottom-sheet bg-white rounded-top-4 shadow-lg d-flex flex-column overflow-hidden position-absolute start-0 w-100"
    ...
    <div class="container-fluid px-3 pt-3 pb-2 flex-shrink-0">
      <div class="row g-2">
        <div class="col-4">
          <div class="bg-primary text-white rounded-4 p-3 d-flex justify-content-center align-items-center shadow-sm h-100">
            <span class="material-symbols-rounded fs-3">location_on</span>
          </div>
        </div>
        <div class="col-4">
          <div class="bg-primary-subtle border border-primary text-primary rounded-4 p-3 d-flex justify-content-center align-items-center h-100">
    ...
    <div id="busListMobile" class="flex-grow-1 overflow-y-auto pb-4">
  ...
  <script>
    function selectNav(element, section) {
      // 1. Reset all buttons: Remove Blue (text-primary), Add Black (text-dark)
      document.querySelectorAll('.nav-btn').forEach(btn => {
        btn.classList.remove('text-primary');
        btn.classList.add('text-dark');
      });

      // 2. Set clicked button: Remove Black, Add Blue
      element.classList.remove('text-dark');
      element.classList.add('text-primary');

      // 3. Logic to Show/Hide the bottom sheet
    ...
    // Auto-reset when modals close (optional polish)
    ...
    const AVG_SPEED_MPS = (AVG_SPEED_KPH * 1000) / 3600;
    ...
    function formatArrivalBySeconds(secondsFromNow) {
      const dt = new Date(Date.now() + Math.max(0, secondsFromNow * 1000));
    ...
    function normalizeBus(bus) {
      let coords = null;
      let locName = bus.current_location_name || 'Updating...';
    ...
    function updateMap(buses) {
      const filtered = buses.filter(b =>
        (!selectedRoute || b.route === selectedRoute) && b.status !== 'unavailable' && b.coords !== null
      );
      ...
      if (userLocation) {
        if (!userMarker) {
          userMarker = L.circleMarker([userLocation.lat, userLocation.lng], { radius: 8, color: '#2563eb', fillColor: '#60a5fa', fillOpacity: 0.9 }).addTo(map);
    ...
    function renderBusList(buses) {
      ...
      if (activeBuses.length === 0) {
        container.innerHTML = `
                <div class="d-flex flex-column justify-content-center align-items-center h-100 text-muted p-5">
                    <span class="material-symbols-rounded fs-1 mb-2">directions_bus_off</span>
          ...
          return `
                <div class="card border-0 border-bottom rounded-0 cursor-pointer" onclick="focusBus('${b.id}')">
                    <div class="card-body py-3 px-4">
                        ...
                        <div class="small text-muted mb-2">${arrivalText}</div>
                        <div class="timeline-container bg-secondary-subtle position-relative">
                            <div class="timeline-progress bg-primary position-absolute top-0 bottom-0 start-0 rounded-pill" style="width: ${progress}%"></div>
                            <span class="material-symbols-rounded timeline-icon position-absolute bg-white rounded-circle text-primary border" style="left: ${progress}%; font-size:18px">directions_bus</span>
                            <span class="material-symbols-rounded timeline-icon stop-point stop-commuter position-absolute bg-white rounded-circle" style="right:6px; transform: translateX(0);">place</span>
                        </div>
                    </div>
                </div>`;
        } else {
          return `
                <button class="list-group-item list-group-item-action" onclick="focusBus('${b.id}')">
                    <div class="d-flex w-100 justify-content-between">
                        <h6 class="mb-1 fw-bold">${b.code}</h6>
                        <small class="badge rounded-pill" style="background:${color}">${b.status}</small>
                    </div>
                    <p class="mb-1 small text-muted text-truncate">${b.locName}</p>
                    <div class="d-flex justify-content-between">
                        <small>${b.seats} Seats</small>
                        <small class="text-muted">${arrivalText}</small>
                    </div>
                </button>`;
        }
      }).join('');
    ...
    function updateFilters(buses) {
      ...
      routes.forEach(r => {
        html += `<button class="list-group-item list-group-item-action ${selectedRoute === r ? 'active' : ''}" onclick="setRoute('${r}')">${r}</button>`;
      });
      list.innerHTML = html;
    }
    ...
    document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        alertBox.innerHTML = '<div class="alert alert-danger py-1">Connection Error</div>';
      }
    });
